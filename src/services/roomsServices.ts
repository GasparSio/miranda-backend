...
async function fetchOne(roomId: string) {
  const query = `select r.*, GROUP_CONCAT(DISTINCT p.photos) AS all_photos, GROUP_CONCAT(a.amenities) AS all_amenities FROM room r LEFT JOIN photo p ON r.id = p.room_id LEFT JOIN amenities_has_room ah ON r.id = ah.room_id LEFT JOIN amenity a ON ah.amenity_id = a.id WHERE r.id =? GROUP BY r.id;`
  const values = [roomId]
  const result = await SelectQuery(query, values)
  return result;
}
